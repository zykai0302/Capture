...
Before following this command, read `_shared/workflow-context.md`. Reuse already-loaded task and plan context instead of re-reading unchanged files for every delegated step.
...
This command integrates the practical parts of subagent-driven development into the Feature task workflow. If the current platform does not support reliable subagents, fall back to `/feature:executing-plans`.
...
1. `/feature:init-plan`
2. `/feature:research`
3. `/feature:before-dev`
4. `/feature:write-plan`
5. `/feature:write-testcase`
6. `/feature:check-testcase`
...
In those cases, use `/feature:executing-plans`.
