...
Before running this command, read `_shared/workflow-context.md`. Reuse unchanged PRD, findings, and task memory that are already loaded in the current task session.
...
This command integrates the strongest parts of `writing-plans`: the plan must be executable by an engineer with little project context, with exact files, validation steps, and no placeholders.
...
```text
/feature:brainstorm -> /feature:init-plan -> /feature:research -> /feature:before-dev -> /feature:write-plan
```
...
- **Display the next command options only:**
  ...
  - /feature:write-testcase
  - /feature:executing-plans 
  - /feature:subagent-work 
  Choose one and run it manually.
  ```

- Ask the user which execution mode they want, but do not run it automatically
- Recommend `/feature:write-testcase` for complex tasks requiring test-driven development

**Failure path:**

- User indicates plan is inaccurate or needs refinement
- Update `task.json` status back to `"brainstorming"`
- Return to `/feature:brainstorm` to re-clarify requirements
- May need additional `/feature:research` if technical details are unclear
...
- for complex tasks requiring TDD: hand off to `/feature:write-testcase`
- for simple tasks: hand off to `/feature:executing-plans` or `/feature:subagent-work`
